...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  :root {
    ...
    /* Code block specific */
    --code-bg:          #07070f;
    --code-bg-header:   rgba(0,0,0,0.25);
  ...
  [data-theme="light"] {
    ...
    --green:       #0d9150;
    --green-dim:   #0a7a42;
    --green-glow:  rgba(13,145,80,0.12);
    --green-trace: rgba(13,145,80,0.07);
    ...
    /* Code block — keeps dark terminal look in light mode (professional) */
    --code-bg:          #1a1a2e;
    --code-bg-header:   rgba(0,0,0,0.35);
  ...
  * { cursor: none !important; }
  ...
  @keyframes glowPulse {
    0%, 100% { box-shadow: 0 0 20px rgba(139,92,246,.3); filter: brightness(1); }
    50% { box-shadow: 0 0 40px rgba(139,92,246,.6); filter: brightness(1.2); }
  }
  ...
  .cursor.hover { transform: scale(1.5); border-color: var(--green); }
  .cursor-dot.hover { background: var(--green); box-shadow: 0 0 15px var(--green); }
  .cursor-trail {
    position: fixed;
  ...
  .packet-core { position: relative; width: 100%; height: 100%; }
  .packet-icon {
    position: absolute;
    inset: 0;
  ...
  .packet-trail::before { top: -4px; }
  .packet-trail::after { top: 4px; }
  .packet-glow {
    position: absolute;
    inset: -20px;
  ...
  /* ── THEME TOGGLE — embedded in sidebar footer ── */
  ...
  .theme-toggle-sidebar:hover .theme-toggle-icon {
  ...
  .sidebar::before {
    ...
    top: 0; left: 0; right: 0;
    height: 400px;
    background: radial-gradient(ellipse at top, rgba(139,92,246,0.07), transparent 70%);
    pointer-events: none;
  }
  .sidebar-logo {
    display: flex;
  ...
  .sidebar-nav { flex: 1; overflow-y: auto; padding: 12px 0 12px; }
  .nav-group-label {
    font-family: var(--font-mono);
    font-size: 0.6rem;
  ...
  .nav-item:hover { color: var(--text-main); background: rgba(139,92,246,.05); transform: translateX(3px); }
  ...
  .nav-dot { width: 5px; height: 5px; border-radius: 50%; background: currentColor; opacity: .4; flex-shrink: 0; transition: all .25s; }
  .nav-item.active .nav-dot { opacity: 1; box-shadow: 0 0 8px currentColor; }
  .sidebar-foot {
  ...
  .sys-status { display: flex; align-items: center; gap: 8px; font-family: var(--font-mono); font-size: 0.62rem; color: var(--text-dim); }
  .sys-dot { width: 7px; height: 7px; border-radius: 50%; background: var(--green); box-shadow: 0 0 8px var(--green); animation: pulse 2s infinite; }
  ...
  /* ── CODE BANNER ── */
  .code-banner-wrapper { position: relative; margin: 50px 0 60px; perspective: 1000px; }
  .code-banner {
    position: relative;
    z-index: 10;
    height: 140px;
    background: linear-gradient(135deg, #0f0f1e, #070710);
  ...
  .code-banner:hover { transform: translateY(-2px); }
  .code-banner-bg {
  ...
  .code-banner-dot { width: 10px; height: 10px; border-radius: 50%; transition: all 0.3s ease; }
  ...
  .code-banner-dot:hover { transform: scale(1.2); box-shadow: 0 0 10px currentColor; }
  .code-banner-stats {
    display: flex; gap: 16px;
    font-family: var(--font-mono); font-size: 0.62rem; color: rgba(255,255,255,0.25);
  }
  .code-banner-stat { display: flex; align-items: center; gap: 4px; }
  .code-banner-stat-dot { width: 4px; height: 4px; border-radius: 50%; background: #00ff88; animation: pulse 2s infinite; }
  .code-banner-content { position: relative; height: calc(100% - 50px); overflow: hidden; }
  .code-banner-lines {
  ...
  .code-line {
    ...
    opacity: 0; animation: codeLineSlide 8s ease-in-out infinite;
  }
  ...
  .code-line-num { color: rgba(255,255,255,0.2); font-size: 0.7rem; min-width: 20px; text-align: right; }
  .code-line-content { display: flex; gap: 8px; align-items: center; }
  /* Banner code uses fixed colors (always dark terminal) */
  .code-keyword-b { color: #c084fc; font-weight: 700; }
  .code-function-b { color: #60a5fa; }
  .code-string-b { color: #86efac; }
  .code-number-b { color: #fbbf24; }
  .code-comment-b { color: rgba(255,255,255,0.3); font-style: italic; }
  .code-operator-b { color: #67e8f9; }
  .code-cursor { display: inline-block; width: 8px; height: 16px; background: #00ff88; animation: blink 1s step-end infinite; margin-left: 2px; }
  .matrix-rain-container { position: absolute; inset: 0; pointer-events: none; overflow: hidden; opacity: 0.15; }
  .matrix-char { position: absolute; font-family: var(--font-mono); font-size: 0.7rem; color: #00ff88; animation: matrixRain linear infinite; }
  .code-banner-particles { position: absolute; inset: 0; pointer-events: none; }
  .banner-particle { position: absolute; width: 3px; height: 3px; background: #8b5cf6; border-radius: 50%; opacity: 0; animation: particleFloat 15s ease-in-out infinite; }

  /* ── TERMINAL CODE BLOCKS — always dark (pro look in both themes) ── */
  .terminal-code-block {
    position: relative;
    background: linear-gradient(135deg, #0d0d1a, #060610);
  ...
  /* Light mode: slightly lighter dark terminal with colored accent border */
  [data-theme="light"] .terminal-code-block {
    background: linear-gradient(135deg, #141428, #0a0a1a);
    border-color: rgba(109,40,217,0.25);
    box-shadow: 0 12px 40px rgba(0,0,0,.25), 0 0 0 1px rgba(109,40,217,0.1), inset 0 1px 0 rgba(255,255,255,.05);
  }
  .terminal-code-bg {
    position: absolute; inset: 0;
    background:
      radial-gradient(circle at 20% 50%, rgba(139,92,246,0.07), transparent 50%),
      radial-gradient(circle at 80% 50%, rgba(0,200,100,0.05), transparent 50%);
    pointer-events: none;
  }
  .terminal-code-grid {
    position: absolute; inset: 0;
  ...
  .terminal-matrix-rain { position: absolute; inset: 0; pointer-events: none; overflow: hidden; opacity: 0.1; z-index: 1; }
  .terminal-matrix-char { position: absolute; font-family: var(--font-mono); font-size: 0.65rem; color: #00ff88; animation: matrixRain linear infinite; }
  .terminal-particles { position: absolute; inset: 0; pointer-events: none; z-index: 1; }
  .terminal-particle { position: absolute; width: 2px; height: 2px; background: #8b5cf6; border-radius: 50%; opacity: 0; animation: particleFloat 12s ease-in-out infinite; }
  .terminal-header {
    display: flex; align-items: center; justify-content: space-between;
    padding: 12px 20px;
    border-bottom: 1px solid rgba(255,255,255,0.05);
  ...
  .terminal-dot { width: 10px; height: 10px; border-radius: 50%; transition: all 0.3s ease; }
  .terminal-dot:nth-child(1) { background: #ff5f57; }
  .terminal-dot:nth-child(2) { background: #ffbd2e; }
  .terminal-dot:nth-child(3) { background: #28c840; }
  .terminal-dot:hover { transform: scale(1.2); box-shadow: 0 0 10px currentColor; }
  .terminal-title { font-family: var(--font-mono); font-size: 0.68rem; color: rgba(255,255,255,0.35); font-weight: 600; letter-spacing: 0.05em; }
  .terminal-tab-row {
    display: flex;
  ...
  .terminal-tab-btn:hover { color: rgba(255,255,255,0.65); }
  .terminal-tab-btn.on { color: #a78bfa; border-bottom-color: #8b5cf6; background: rgba(139,92,246,0.1); font-weight: 600; }
  ...
  .terminal-copy-btn {
    ...
    transition: all .2s; letter-spacing: .05em; font-weight: 600; z-index: 20;
  }
  .terminal-copy-btn:hover { color: #a78bfa; border-color: rgba(139,92,246,.4); background: rgba(139,92,246,.1); transform: translateY(-1px); }
  .terminal-copy-btn.done { color: #34d399; border-color: rgba(52,211,153,.4); background: rgba(52,211,153,.08); }
  .terminal-pre {
    padding: 24px;
    overflow-x: auto;
    font-family: var(--font-mono); font-size: 0.78rem; line-height: 1.85;
    color: var(--code-text);
    position: relative;
  }

  /* ── SYNTAX HIGHLIGHT — uses CSS vars set per theme ── */
  .kw  { color: var(--code-key); }
  .st  { color: var(--code-string); }
  .nm  { color: var(--code-number); }
  .cm  { color: var(--code-comment); font-style: italic; }
  .ky  { color: var(--code-ident); }
  .fn  { color: var(--code-fn); }
  .bl  { color: var(--code-bool); }
  .ur  { color: var(--code-url); }
  .hd  { color: var(--code-header-txt); }
  ...
  .topbar-bc { display: flex; align-items: center; gap: 8px; font-family: var(--font-mono); font-size: 0.7rem; color: var(--text-dim); }
  ...
  .tb-btn::before { content: ''; position: absolute; inset: 0; background: linear-gradient(135deg, var(--purple-glow), transparent); opacity: 0; transition: opacity .25s; }
  .tb-btn:hover::before { opacity: 1; }
  .tb-btn:hover { color: var(--purple); border-color: rgba(109,40,217,.4); transform: translateY(-1px); }
  .tb-btn.cta {
    background: linear-gradient(135deg, var(--purple), var(--purple-dim));
    color: white; border-color: transparent;
    font-weight: 700;
    box-shadow: 0 0 20px rgba(109,40,217,.25), 0 4px 12px rgba(0,0,0,.15);
  }
  .tb-btn.cta::before { background: linear-gradient(135deg, rgba(255,255,255,.12), transparent); }
  .tb-btn.cta:hover { box-shadow: 0 0 30px rgba(109,40,217,.4), 0 6px 20px rgba(0,0,0,.2); transform: translateY(-2px); }

  /* ── CONTENT ── */
  .content { padding: 60px 56px 100px; max-width: 1000px; animation: fadeIn .6s ease; }
  ...
  .hero-glow { position: absolute; width: 600px; height: 400px; background: radial-gradient(ellipse, rgba(139,92,246,.1), transparent 70%); top: -80px; left: -140px; pointer-events: none; animation: float 8s ease-in-out infinite; }
  .hero-glow-2 { position: absolute; width: 400px; height: 300px; background: radial-gradient(ellipse, rgba(0,200,100,.07), transparent 70%); bottom: -60px; right: -100px; pointer-events: none; animation: float 10s ease-in-out infinite reverse; }
  .hero-logo-float { position: absolute; right: -50px; top: 50%; transform: translateY(-50%); width: 280px; height: 280px; opacity: 0.07; animation: float 12s ease-in-out infinite; pointer-events: none; }
  .hero-logo-float svg { width: 100%; height: 100%; filter: drop-shadow(0 0 40px rgba(139,92,246,.3)); }
  .hero-tag {
  ...
  .hero-tag-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--purple); box-shadow: 0 0 8px var(--purple); animation: pulse 1.6s infinite; }
  .hero-h1 {
    font-size: clamp(2.2rem, 4.2vw, 3.8rem);
    font-weight: 800; letter-spacing: -.05em; line-height: 1.08;
    color: var(--text-bright); margin-bottom: 20px;
    position: relative; animation: slideInFromLeft 0.8s ease-out 0.3s backwards;
  }
  ...
  .hero-sub { font-family: var(--font-mono); font-size: 0.88rem; color: var(--text-dim); line-height: 1.8; max-width: 600px; margin-bottom: 32px; position: relative; animation: slideInFromLeft 0.8s ease-out 0.4s backwards; }
  .base-chip {
  ...
  .base-chip-label { padding: 10px 16px; background: var(--bg-elevated); color: var(--text-faint); font-size: 0.62rem; text-transform: uppercase; letter-spacing: .12em; border-right: 1px solid var(--border-dim); font-weight: 600; }
  .base-chip-val { padding: 10px 18px; color: var(--purple); font-weight: 600; }

  /* ── FEATURE GRID ── */
  .feat-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 16px; margin-bottom: 60px; }
  .feat {
    background: var(--bg-card);
  ...
  .feat::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at top right, var(--purple-glow), transparent 60%); opacity: 0; transition: opacity .3s; }
  .feat:hover { border-color: rgba(109,40,217,.25); background: var(--bg-elevated); transform: translateY(-4px); box-shadow: 0 12px 40px rgba(109,40,217,.12); }
  .feat:hover::before { opacity: 1; }
  .feat-icon { font-size: 1.4rem; margin-bottom: 12px; display: block; position: relative; z-index: 1; }
  .feat-title { font-size: 0.82rem; font-weight: 700; color: var(--text-bright); margin-bottom: 6px; letter-spacing: -.02em; position: relative; z-index: 1; }
  .feat-desc { font-family: var(--font-mono); font-size: 0.68rem; color: var(--text-dim); line-height: 1.65; position: relative; z-index: 1; }

  /* ── SECTION HEADER ── */
  .sec-head { margin-bottom: 26px; }
  .sec-eye { font-family: var(--font-mono); font-size: 0.6rem; text-transform: uppercase; letter-spacing: .18em; color: var(--purple); margin-bottom: 6px; font-weight: 600; }
  .sec-h2 { font-size: 1.6rem; font-weight: 800; letter-spacing: -.04em; color: var(--text-bright); }
  .sec-rule { width: 40px; height: 3px; background: linear-gradient(90deg, var(--purple), var(--green)); border-radius: 2px; box-shadow: 0 0 10px rgba(109,40,217,.3); margin-top: 12px; }

  /* ── ENDPOINT / TABLE BLOCKS ── */
  .ep-block { background: var(--bg-card); border: 1px solid var(--border-subtle); border-radius: 14px; overflow: hidden; margin-bottom: 32px; box-shadow: 0 8px 32px rgba(0,0,0,.08); transition: all .3s; }
  .ep-block:hover { box-shadow: 0 12px 48px rgba(109,40,217,.1); transform: translateY(-2px); }
  .ep-bar { display: flex; align-items: center; gap: 14px; padding: 14px 22px; background: var(--bg-elevated); border-bottom: 1px solid var(--border-faint); }
  .m-tag { font-family: var(--font-mono); font-size: 0.65rem; font-weight: 700; padding: 4px 10px; border-radius: 6px; letter-spacing: .06em; }
  .m-post { background: rgba(29,78,216,0.12); color: var(--blue); border: 1px solid rgba(29,78,216,.25); }
  ...
  .tbl-lbl { font-family: var(--font-mono); font-size: 0.6rem; text-transform: uppercase; letter-spacing: .16em; color: var(--text-faint); margin-bottom: 14px; font-weight: 600; }
  ...
  tbody tr:hover td { background: rgba(109,40,217,.04); }
  .p-name { color: var(--blue); font-weight: 600; }
  .p-type { color: var(--amber); }
  .p-def { color: var(--text-faint); font-style: italic; }
  .p-req { font-size: 0.56rem; background: var(--red-glow); color: var(--red); border: 1px solid rgba(220,38,38,.3); padding: 2px 6px; border-radius: 4px; margin-left: 6px; vertical-align: middle; white-space: nowrap; font-weight: 700; }
  .f-name { color: var(--blue); font-weight: 600; }

  /* ── STATUS CODES ── */
  .sc-row { display: grid; grid-template-columns: repeat(4,1fr); gap: 12px; margin-bottom: 36px; }
  .sc { background: var(--bg-card); border: 1px solid var(--border-faint); border-radius: 10px; padding: 16px; display: flex; align-items: center; gap: 12px; transition: all .3s; position: relative; overflow: hidden; }
  .sc:hover { border-color: var(--border-dim); transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,.08); }
  .sc-code { font-family: var(--font-mono); font-size: 1.1rem; font-weight: 700; min-width: 42px; position: relative; z-index: 1; }
  .c200 { color: var(--green); }
  .c400 { color: var(--amber); }
  .c429 { color: var(--red); }
  .c504 { color: var(--red-mid); }
  .sc-msg { font-family: var(--font-mono); font-size: 0.68rem; color: var(--text-dim); line-height: 1.5; position: relative; z-index: 1; }

  /* ── GUIDES ── */
  .guide { background: var(--bg-card); border: 1px solid var(--border-faint); border-radius: 14px; padding: 28px; margin-bottom: 18px; position: relative; overflow: hidden; transition: all .3s; }
  .guide:hover { border-color: rgba(109,40,217,.2); transform: translateY(-2px); box-shadow: 0 12px 40px rgba(109,40,217,.08); }
  .guide-num { font-family: var(--font-mono); font-size: 0.6rem; color: var(--purple); background: var(--purple-glow); border: 1px solid rgba(109,40,217,.2); padding: 3px 10px; border-radius: 5px; display: inline-block; margin-bottom: 12px; letter-spacing: .1em; font-weight: 700; }
  ...
  .warn { display: flex; gap: 10px; align-items: flex-start; background: var(--amber-glow); border: 1px solid rgba(180,83,9,.2); border-radius: 10px; padding: 12px 14px; margin-top: 14px; font-family: var(--font-mono); font-size: 0.72rem; color: var(--amber); line-height: 1.7; }
  ...
  .json-bar-ok { font-family: var(--font-mono); font-size: 0.6rem; color: var(--green); background: var(--green-trace); border: 1px solid rgba(13,145,80,.2); padding: 2px 8px; border-radius: 4px; font-weight: 600; }
  .key-notice { display: inline-flex; align-items: center; gap: 10px; font-family: var(--font-mono); font-size: 0.68rem; color: var(--amber); background: var(--amber-glow); border: 1px solid rgba(180,83,9,.2); padding: 7px 14px; border-radius: 8px; margin-bottom: 20px; }
  ...
  .divider::before { content: ''; position: absolute; left: 50%; top: 50%; transform: translate(-50%,-50%); width: 8px; height: 8px; background: var(--purple); border-radius: 50%; box-shadow: 0 0 20px rgba(109,40,217,.5); }

  /* ── CTA ── */
  .cta-block { text-align: center; padding: 56px 0 24px; border-top: 1px solid var(--border-faint); margin-top: 56px; position: relative; }
  .cta-block::before { content: ''; position: absolute; top: -100px; left: 50%; transform: translateX(-50%); width: 400px; height: 200px; background: radial-gradient(ellipse, rgba(109,40,217,.1), transparent 70%); pointer-events: none; }
  ...
  .btn-p { display: inline-flex; align-items: center; gap: 8px; background: linear-gradient(135deg, var(--purple), var(--purple-dim)); color: white; font-family: var(--font-mono); font-size: 0.76rem; font-weight: 700; padding: 12px 26px; border-radius: 10px; text-decoration: none; letter-spacing: .05em; box-shadow: 0 0 24px rgba(109,40,217,.3), 0 6px 16px rgba(0,0,0,.15); transition: all .3s; border: none; position: relative; overflow: hidden; }
  .btn-p:hover { box-shadow: 0 0 40px rgba(109,40,217,.5), 0 8px 24px rgba(0,0,0,.2); transform: translateY(-3px); }
  .btn-s { display: inline-flex; align-items: center; gap: 8px; background: transparent; color: var(--text-dim); font-family: var(--font-mono); font-size: 0.76rem; padding: 12px 26px; border-radius: 10px; text-decoration: none; letter-spacing: .05em; border: 1px solid var(--border-dim); transition: all .3s; }
  .btn-s:hover { color: var(--text-main); border-color: var(--border-active); transform: translateY(-1px); }

  /* ── FOOTER ── */
  .footer { border-top: 1px solid var(--border-faint); padding: 30px 56px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 16px; background: var(--bg-deep); transition: background 0.35s ease; }
  ...
  .footer-link { font-family: var(--font-mono); font-size: 0.65rem; color: var(--text-faint); padding: 6px 12px; border: 1px solid var(--border-faint); border-radius: 7px; background: none; transition: all .2s; text-decoration: none; display: inline-block; }
  .footer-link:hover { color: var(--purple); border-color: rgba(109,40,217,.3); background: var(--purple-glow); }
...
// Syntax highlighter — outputs spans using CSS-var-based classes
...
// Banner code lines — always use fixed inline dark-terminal color classes
const CODE_LINES = [
  { num: 1, code: [
    { cls: 'code-keyword-b', text: 'const' },
    { cls: '', text: ' response = ' },
    { cls: 'code-keyword-b', text: 'await' },
    { cls: 'code-function-b', text: ' fetch' },
    { cls: '', text: '(' },
    { cls: 'code-string-b', text: '"https://og-miner.p.rapidapi.com/v1/extract"' },
    { cls: '', text: ')' },
  ]},
  { num: 2, code: [
    { cls: 'code-keyword-b', text: 'async' },
    { cls: 'code-function-b', text: ' extractMetadata' },
    { cls: '', text: '(url) => { ' },
    { cls: 'code-keyword-b', text: 'return' },
    { cls: 'code-function-b', text: ' parse' },
    { cls: '', text: '(url) }' },
  ]},
  { num: 3, code: [{ cls: 'code-comment-b', text: '// OpenGraph extraction with caching layer' }]},
  { num: 4, code: [
    { cls: 'code-keyword-b', text: 'POST' },
    { cls: 'code-string-b', text: ' /v1/extract' },
    { cls: 'code-operator-b', text: ' → ' },
    { cls: 'code-number-b', text: '200' },
    { cls: '', text: ' OK ' },
    { cls: 'code-operator-b', text: '⚡' },
    { cls: 'code-number-b', text: ' 45ms' },
  ]},
];
...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (cursor) cursor.style.transform = `translate(${cursorX - 10}px, ${cursorY - 10}px)`;
      if (cursorDot) cursorDot.style.transform = `translate(${dotX - 3}px, ${dotY - 3}px)`;
      if (Math.random() > 0.8) {
    ...
    const handleMouseEnter = (e) => {
      const t = e.target;
      if (t.tagName === 'BUTTON' || t.tagName === 'A' || t.classList.contains('nav-item')) setIsHovering(true);
    };
...
// ─── Code Banner (always dark terminal) ─────────────────────────────────────
function AdvancedCodeBanner() {
  useEffect(() => {
    ...
    const iv = setInterval(createChar, 200);
    ...
    <div className="code-banner-wrapper">
      <div className="code-banner">
        ...
        <div className="code-banner-header">
          ...
          <div className="code-banner-title"><span>⚡</span><span>og-miner-api.js</span></div>
          <div className="code-banner-stats">
            <div className="code-banner-stat"><div className="code-banner-stat-dot" /><span>LIVE</span></div>
            <div className="code-banner-stat"><span>45ms avg</span></div>
          </div>
        </div>
...
function TerminalCodeTabs({ tabs, title }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(createChar, 250);
    return () => clearInterval(iv);
  }, []);
...
// ─── Terminal Pre (single code block) ───────────────────────────────────────
    ...
    <>
      ...
      <div className="shell">
        ...
        <aside className="sidebar">
          ...
          {/* Theme toggle lives here — inside sidebar footer, no overlap */}
        ...
        <div className="main" ref={mainRef}>
          ...
          <div className="content">
            ...
            <div className="hero" id="intro" style={{ scrollMarginTop: 100 }}>
              ...
              <div className="hero-logo-float"><LogoSvg /></div>
            ...
            <div id="g-link" style={{ scrollMarginTop: 100 }}>
              ...
              <TerminalPre title="LinkCard.jsx" code={LINK_CODE} lang="js" />
            </div>
            ...
            <div id="g-spa" style={{ scrollMarginTop: 100 }}>
              ...
              <TerminalPre title="spa-scraping.js" code={SPA_CODE} lang="js" />
              <div className="guide">
                <div className="warn">
                  <span>⚠</span>
